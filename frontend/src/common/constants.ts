...
export class NetConfig {
    public static readonly SSL_ENABLED = true; // Set to true if using HTTPS
    public static readonly PORT = 5800; // Port for the WebSocket server
    public static readonly SERVER_URL = NetConfig.SSL_ENABLED ? 
        `wss://${window.location.hostname}:${NetConfig.PORT}` : 
        `ws://${window.location.hostname}:${NetConfig.PORT}`;
    public static readonly API_URL = NetConfig.SSL_ENABLED ?
        `https://${window.location.hostname}:${NetConfig.PORT}` :
        `http://${window.location.hostname}:${NetConfig.PORT}`;
}
